import React from 'react';
export const page1WithImg = (
  <div className = "row team-task-box">

    <div id="demo" className="border-box">
      <div>
        <h3 className="centerdiv"><u><b>Task</b></u></h3>
      </div>
      <br/>
      <br/>
      <div className="paragraph">
        <u><b>Write a television advertisement for the transportation device you described in the earlier task.</b></u>
        <br></br>

        <p>- the television advertisement should be 30 seconds</p>
        <p>- it should highlight the product's main benefits and ultimately persuade people to buy it</p>
        <p>- you can make up a new name and invent realistic features that it should have </p>
        <br></br>
        <b>You have 1:00 minute</b> to read these directions before you're automatically moved to the next section. Scroll to the bottom of the page to see all of the directions and the photo.
        <br/>
        <br/>
        <p><u><b>Sample advertisement</b></u></p>
      </div>
      <div className="tab">

        <p>Television advertisement for a new type of beverage:</p>

        <p> "Scopa: Live life, one bean at a time A rugged man is free-climbing up Puncak Jaya, the highest mountain in New Guinea. He climbs effortlessly and quickly reaches the summit. Once at the top he takes a moment to admire the view surrounded by clouds and then removes a flask from his backpack, swiftly pouring a brown liquid into the cup of the flask. Last, the scene cuts to a blank screen with writing saying: “2 beans, 2 sides, which side will you be today?”
        </p>

      </div>
      <br/>
      <div className="centerdiv">
        <img
          src="http://www.electricvelocity.com.au/Upload/Blogs/smart-e-bike-side_2.jpg"/>
      </div>
    </div>
  </div>
);

export const page1 = (
  <div className = "row">

    <div id="demo">
      <div>
        <h3 className="centerdiv"><u><b>Task</b></u></h3>
      </div>

      <div>
        <u><b>Write a television advertisement for the transportation device you described in the earlier task.</b></u>
        <br></br>

        <p>- the television advertisement should be 30 seconds</p>
        <p>- it should highlight the product 's main benefits and ultimately persuade people to buy it</p>
        <p>- you can make up a new name and invent realistic features that it should have </p>
        <br></br>
        <b>You have 1:00 minute</b> to read these directions before you 're automatically moved to the next section. Scroll to the bottom of the page to see all of the directions and the photo.

        <p><u><b>Sample advertisement</b></u></p>

        <p>Television advertisement for a new type of beverage:</p>

        <p> "Scopa: Live life, one bean at a time A rugged man is free-climbing up Puncak Jaya, the highest mountain in New Guinea. He climbs effortlessly and quickly reaches the summit. Once at the top he takes a moment to admire the view surrounded by clouds and then removes a flask from his backpack, swiftly pouring a brown liquid into the cup of the flask. Last, the scene cuts to a blank screen with writing saying: “2 beans, 2 sides, which side will you be today?”
        </p>
      </div>
    </div>
  </div>
);

export const page2 = (

  <div id = "demo" className = "row team-task-box">
    <div>
      <h3 className="centerdiv"><u><b>Teamwork Directions</b></u></h3>
    </div>


    <p className="page2-intro"><b>You have 1:00 minute </b> to read these directions before you 're automatically moved to the first part of the task. There are 4 sections to the the task: work on each section in the appropriate area on the collaborative workspace.<br></br>
    <br/>
    Important : <br/>
    <b> DO NOT COPY AN ADVERTISEMENT FROM ELSEWHERE ON WEB</b><br/>
    <b>DO NOT REFRESH YOUR WEBPAGE DURING THE TASK</b></p>

    <br/>
    <br/>
    <div  >
      <h4><b>Part 1: FEATURE LIST</b></h4>
      <ul>
        <li><p><b>You have 2 minutes. </b>
        Write your name under “team member” at the top of the collaborative workspace.
        </p></li>
        <li><p><b>Introduce yourself to your teammate </b>by writing anywhere in the collaborative workspace.</p></li>
        <li><p>Read your own and your teammate's slogans written during the individual task.</p></li>
        <li><p><b>Write down</b> all your ideas for features of the transportation device<b> as a list. </b>Record them under “Part 1” heading of the collaborative workspace.
        </p></li>
      </ul>
    </div>

    <div  >
      <h4><b>Part 2: FEATURE SELECTION</b></h4>
      <ul>
        <li><p><b>You have 1.5 minutes.</b></p></li>
        <li><p><b>As a team, select the best features you want to use for the advertisement. </b>
        Combine your ideas and build off each other. Discuss the merits of your individual ideas come up with ways to merge them.
        </p></li>
        <li><p>Record the features under “Part 2” heading of the collaborative workspace.</p> </li>
      </ul>
    </div>

    <div >
      <b><h4>Part 3: WRITING ADVERTISEMENT</h4></b>
      <ul>
        <li><p><b>You have 5 minutes</b></p></li>
        <li><p><b>The television advertisement should have a description of the visuals of the advertisement and the action that is happening. </b> Feel free to add additional features, for example “voiceover”, “text on screen”, “speaking”, or “music”. See the sample advertisement under “Task” for an example.
        </p></li>
        <li><p> Write your advertisement <b>TOGETHER. Each teammate must contribute at least a third of the work</b> - in term of words, ideas, suggestions - we want to see you combining ideas in real time.
        </p></li>
        <li><p> Do the work under the “Part 3” heading of the collaborative workspace.</p></li>
    </ul>
    </div>

    <div >
      <b><h4>Part 4: FINALZING ADVERTISEMENT</h4></b>
      <ul>
        <li><p><b>You have 1.5 minutes.</b></p></li>
        <li><p><b>Finalize your advertisement by editing what you've written.</b></p></li>
        <li><p><b>Paste the final version under the “Part 4” heading</b> of the collaborative workspace.
        </p></li>
      </ul>
    </div>

  </div>
);

export const page3 = (
  <div className = "row" id = "demo" >
    <h4><b>Part 1: FEATURE LIST</b></h4>
    <ul>
      <li><p><b>You have 2 minutes. </b>
      Write your name under “team member” at the top of the collaborative workspace.
      </p></li>
      <li><p><b>Introduce yourself to your teammate
      </b> by writing anywhere in the collaborative workspace.</p></li>
      <li><p>Read your own and your teammate's slogans written during the individual task.</p></li>
      <li><p><b>Write down</b> all your ideas for features of the transportation device<b> as a list.
      </b> Record them under “Part 1” heading of the collaborative workspace.
      </p></li>
    </ul>
  </div>

);

export const page4 = (
  <div className = "row" id = "demo" >
    <h4><b>Part 2: FEATURE SELECTION</b></h4>
    <ul>
      <li><p><b>You have 1.5 minutes.</b></p></li>
      <li><p><b>As a team, select the best features you want to use for the advertisement. </b>
      Combine your ideas and build off each other. Discuss the merits of your individual ideas come up with ways to merge them.
      </p></li>
      <li><p>Record the features under “Part 2” heading of the collaborative workspace.</p> </li>
    </ul>
  </div>
);

export const page5 = (
  <div className="row" id="demo">
    <b><h4>Part 3: WRITING ADVERTISEMENT</h4></b>
    <ul>
      <li><p><b>You have 5 minutes</b></p></li>
      <li><p><b>The television advertisement should have a description of the visuals of the advertisement and the action that is happening. </b> Feel free to add additional features, for example “voiceover”, “text on screen”, “speaking”, or “music”. See the sample advertisement under “Task” for an example.
      </p></li>
      <li><p> Write your advertisement <b>TOGETHER. Each teammate must contribute at least a third of the work</b> - in term of words, ideas, suggestions - we want to see you combining ideas in real time.
      </p></li>
      <li><p> Do the work under the “Part 3” heading of the collaborative workspace.</p></li>
  </ul>
  </div>
);

export const page6 = (
  <div className="row" id="demo">
    <b><h4>Part 4: FINALZING ADVERTISEMENT</h4></b>
    <ul>
      <li><p><b>You have 1.5 minutes.</b></p></li>
      <li><p><b>Finalize your advertisement by editing what you've written.</b></p></li>
      <li><p><b>Paste the final version under the “Part 4” heading</b> of the collaborative workspace.
      </p></li>
    </ul>
  </div>
);
